...
#define MAIN_TEST_DELAY_MS    (50)

int main(void)
{
    openart_pose_t openart_pose = {0};
    openart_map_t openart_map = {0};
    openart_yellow_boxes_t yellow_boxes = {0};
    uint8 i;
    ...
    while(1)
    {
        openart_uart_update(&openart_pose, &openart_map, &yellow_boxes);

        if(yellow_boxes.updated)
        {
            printf("\n=== Yellow Boxes Received ===\n");
            printf("seq=%u count=%u valid=%u\n",
                   yellow_boxes.seq, yellow_boxes.count, yellow_boxes.valid);

            if(yellow_boxes.valid && yellow_boxes.count > 0)
            {
                for(i = 0; i < yellow_boxes.count; i++)
                {
                    printf("  box[%u]: x10=%d y10=%d\n",
                           i, yellow_boxes.x[i], yellow_boxes.y[i]);
                }
            }
            else
            {
                printf("  (no yellow boxes found)\n");
            }

            openart_uart_clear_updated(&openart_pose, &openart_map, &yellow_boxes);
        }
