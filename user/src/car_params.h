 ...
 *
 * 例如 100 就是原来的 100%，给 120 就是这个轮子单独 120% 输出。
 ...
 * gyro_z_angle.c 参数
 *
 * GYRO_Z_DEAD_ZONE_RAW：
 *     gyro_z 原始值在 (-死区, +死区) 范围内时认为没有旋转。
 *
 * GYRO_Z_OUTPUT_SCALE：
 *     输出角度值 = 原始积分值 * 缩放系数。
 *
 * GYRO_Z_INTEGRAL_LIMIT_RAW：
 *     原始积分值限幅；0 表示不限制。
 */
#define GYRO_Z_DEAD_ZONE_RAW                (6)
#define GYRO_Z_OUTPUT_SCALE                 (0.001f)
